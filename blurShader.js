...
BlurShader.prototype.VertexPosition = function(builtIns) { 
    this.varyings.vTextureCoord = this.attributes.aTextureCoord;
...
BlurShader.prototype.FragmentColor = function(builtIns) {
    ...
    var fmx = builtIns.mod(this.varyings.vTextureCoord[0] * 3.0, 1.0);
    var fmy = builtIns.mod(this.varyings.vTextureCoord[1] * 3.0, 1.0);

    // floor((m - 5) / 2) + 1
    // 29 - 5 = 24 / 2 = 12 + 1 = 13 possible positions for the kernel within the source texture
    // destination (0-13)
    var dx = (fmx * 15.0); // WTF? 15 should be 13
    var dy = (fmy * 15.0);
    ...
    var k = sz*sz; 
    return [val[0]/k, val[1]/k, val[2]/k, val[3]/k];
} 
